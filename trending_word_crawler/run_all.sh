...
if [ ! -f "word_list.json" ]; then
    echo "[1/6] 메인 페이지에서 유행어 목록 추출 중..."
    "$PYTHON" step1_get_word_list.py
else
    echo "[1/6] word_list.json 이미 있음 - 건너뜀"
fi

# Step 2: 각 단어 페이지에서 본문 텍스트 추출
echo ""
echo "[2/6] 각 단어 페이지 본문 크롤링 (약 38분 소요)..."
"$PYTHON" step2_crawl_descriptions.py

# Step 3: 최종 병합 및 저장
echo ""
echo "[3/6] 최종 데이터 저장..."
"$PYTHON" step3_save_final.py

# Step 4: OpenAI로 뜻/감성 정제
echo ""
echo "[4/6] OpenAI 정제 실행..."
"$PYTHON" step4_enrich_with_openai.py

# Step 5: S3 업로드
echo ""
echo "[5/6] S3 업로드..."
"$PYTHON" step5_upload_to_s3.py

# Step 6: DB 적재
echo ""
echo "[6/6] DB 적재..."
"$PYTHON" step6_load_enriched_to_db.py
